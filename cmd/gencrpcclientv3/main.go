...
func main() {
	...
	neededImports["client"] = &ImportInfo{Path: "\"github.com/ndsky1003/crpc/v3/client\"", Alias: "client"}
...
func (m GenFuncInfo) ReturnSignature() string {
	...
	default: // Call
		...
		// 【修改点 1】: 强制追加 error，无论原方法是否有 error 返回值
		// 这样生成的 Client 存根总是返回 error，避免吞掉错误。
...
func {{.FuncName}}({{.ParamsSignature}}) {{.ReturnSignature}} {
	...
	{{- else}}
		...
		// 【修正】强制捕获 error
		err := {{.ClientVar}}.Call(ctx, "{{.Service}}", "{{.Module}}.{{.Method}}", reqBody, resPtr, opts...)
